...
export default function IncomePage() {
  ...
  // Calculate totals
  const monthlyTotal = incomeSources.reduce((sum, source) => sum + (source.monthly_total || 0), 0)
  ...
  const fetchIncomeSources = async () => {
    ...
    try {
      ...
      // Get recent entries for each source
      const sourcesWithEntries: IncomeSourceWithEntries[] = []
      
      for (const source of sources || []) {
        // Get recent entries (last 3 months)
        ...
        const monthly_total = monthlyEntries.reduce((sum, entry) => sum + entry.amount, 0)
        ...
        <div>
          ...
          {incomeSources.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-lg shadow">
              ...
              <h3 className="mt-4 text-sm font-medium text-gray-900">No income sources yet</h3>
